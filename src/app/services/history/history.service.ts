...
export class HistoryService {
  ...
  transfer_cart_to_history(userid, date, canteenid, foodid, foodname, foodprice:number, image, orderquantity:number, vendorid){
    
    
    return this.firestore.collection("history").doc(userid).collection('data').add({canteenid: canteenid, date: date, foodid: foodid,
    foodname: foodname, foodprice: foodprice, image: image, orderquantity: orderquantity, vendorid: vendorid});
  }
